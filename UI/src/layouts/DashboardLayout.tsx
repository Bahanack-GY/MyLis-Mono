...
const ReminderPopup = ({ onClose }: { onClose: () => void }) => {
  ...
  animate={{ opacity: 1 }}
  exit={{ opacity: 0 }}
  className="fixed inset-0 z-190 flex justify-end bg-black/30"
  onClick={onClose}
  >
  <motion.div
  role="dialog"
  ...
  <p className="text-sm text-[#1c2b3a] font-medium leading-snug line-clamp-2">{r.title}</p>
  </div>
  );
  })}
  </div>
  <div className="px-4 py-3 border-t border-[#e5e8ef] flex justify-end">
  <button
...
const WelcomeModal = ({ firstName, onClose }: { firstName: string; onClose: () => void }) => {
  ...
  <p className="text-[#8892a4] leading-relaxed mb-2">
  ...
  <button
  onClick={onClose}
  className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-linear-to-r from-[#33cbcc] to-[#2196F3] transition-shadow"
  >
  C'est parti !
  </button>
  </div>
...
const BirthdayModal = ({ people, onClose }: { people: BirthdayEmployee[]; onClose: () => void }) => {
  ...
  transition={{ type: 'spring', stiffness: 320, damping: 24 }}
  onClick={e => e.stopPropagation()}
  className="bg-white w-full max-w-sm border border-[#e5e8ef] shadow-xl overflow-hidden"
  >
  {/* Accent top */}
...
const QuoteModal = ({ onClose }: { onClose: () => void }) => {
  ...
  <div className="px-8 py-6 text-center">
  <blockquote className="text-[#1c2b3a] text-base leading-relaxed italic mb-4">
 "{quote.text}"
  </blockquote>
  ...
  <button
  onClick={onClose}
  className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-[#283852] hover:bg-[#1e2d42] transition-colors cursor-pointer"
  >
