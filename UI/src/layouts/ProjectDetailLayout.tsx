...
const EditProjectModal = ({ project, onClose }: { project: Project; onClose: () => void }) => {
  ...
  <p className="text-xs text-[#8892a4] py-2">{t('projects.noServices', 'No active services for this department')}</p>
  ) : (
  ...
  {s.price != null && (
  <span className="text-xs text-[#8892a4]">{new Intl.NumberFormat('fr-FR').format(s.price)} FCFA</span>
  )}
  </label>
  ))}
  </div>
  ...
  className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white transition-colors shadow-lg shadow-[#33cbcc]/20 ${
  isValid ? 'bg-[#33cbcc] hover:bg-[#2bb5b6]' : 'bg-[#b0bac9] cursor-not-allowed shadow-none'
  }`}
  >
