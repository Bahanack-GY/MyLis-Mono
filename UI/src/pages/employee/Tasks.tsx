...
const KanbanCard = ({
  ...
  {task.leadCode && (
  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#283852]/10 text-[#283852]  text-[10px] font-semibold mt-1">
  <Target01Icon size={9} />
  {task.leadCode}
  </span>
  )}
...
const SubtasksSection = ({ task, onAllCompleted }: { task: MappedTask; onAllCompleted?: () => void }) => {
  ...
  className="flex-1 text-sm text-[#1c2b3a] bg-[#f8f9fc] border border-[#e5e8ef] px-3 py-2 focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors"
  />
  <button
...
const EditSelfTaskModal = ({
  ...
  const inputCls = 'w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-2.5 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors';
  ...
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5">{t('tasks.selfAssign.titleLabel')}</label>
  <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className={inputCls} />
  </div>
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5">{t('tasks.selfAssign.descriptionLabel')}</label>
  <RichTextEditor value={form.description} onChange={html => setForm(f => ({ ...f, description: html }))} />
  </div>
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-2">{t('tasks.selfAssign.difficultyLabel')}</label>
  ...
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Briefcase01Icon size={10} />{t('tasks.selfAssign.projectLabel')}</label>
  ...
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Tag01Icon size={10} />{t('tasks.selfAssign.natureLabel')}</label>
  ...
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Target01Icon size={10} />{t('tasks.selfAssign.leadLabel', 'Lead')}</label>
  ...
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Calendar01Icon size={10} />{t('tasks.selfAssign.startDateLabel')}</label>
  <input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} className={inputCls} />
  </div>
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Calendar01Icon size={10} />{t('tasks.selfAssign.endDateLabel')}</label>
  <input type="date" value={form.endDate} onChange={e => setForm(f => ({ ...f, endDate: e.target.value }))} className={inputCls} />
  </div>
  <div>
  <label className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-widest mb-1.5"><Clock01Icon size={10} />{t('tasks.selfAssign.timeLabel')}</label>
  ...
  <button onClick={onClose} className="flex-1 py-3 text-sm font-semibold text-[#8892a4] border border-[#e5e8ef] hover:border-[#283852] hover:text-[#283852] transition-colors">{t('tasks.selfAssign.cancel')}</button>
  <button
...
const TaskHistoryModal = ({ taskId, onClose }: { taskId: string; onClose: () => void }) => {
  ...
  <p className="text-center text-sm text-[#8892a4] py-8">{t('tasks.history.empty')}</p>
  ) : (
  <div className="space-y-4">
  {(history as any[]).map((entry: any) => (
  <div key={entry.id} className="bg-[#f8f9fc] border border-[#e5e8ef] p-4 space-y-2">
...
const WeeklyComplianceBlockModal = ({ pendingTasks, onClose }: { pendingTasks: Task[]; onClose: () => void }) => {
  ...
  className="bg-white w-full max-w-sm h-full flex flex-col border-l border-[#e5e8ef]"
  >
  <div className="h-1 bg-[#283852] shrink-0" />
  ...
  <div key={task.id} className="bg-[#f8f9fc] border border-[#e5e8ef] p-3">
  <div className="flex items-start justify-between gap-2">
  <h4 className="text-sm font-semibold text-[#1c2b3a] truncate">{task.title}</h4>
  <span className="text-[10px] font-semibold px-2 py-0.5 bg-[#283852]/10 text-[#283852] shrink-0">
  {task.state}
  </span>
  </div>
  {(task.startDate || task.endDate) && (
  <div className="flex items-center gap-1 text-[11px] text-[#8892a4] mt-1">
  ...
  <button
  onClick={onClose}
  className="flex-1 py-3 text-sm font-semibold text-white bg-[#283852] hover:bg-[#1e2d42] transition-colors"
  >
...
const Tasks = () => {
  ...
  className="flex items-center gap-2 bg-[#33cbcc] text-white px-4 py-2.5 text-sm font-semibold hover:bg-[#2bb5b6] transition-colors shrink-0"
  >
  <Add01Icon size={16} />
  ...
  className="border border-[#e5e8ef] overflow-hidden cursor-pointer"
  >
  ...
  : 'bg-white text-[#8892a4] border border-[#e5e8ef] hover:border-[#33cbcc]/50'
  }`}
  >
  {sf.label}
  </button>
  ...
  <div ref={customPickerRef} className="flex items-center gap-2 bg-white border border-[#e5e8ef] px-3 py-2 w-fit ml-auto">
  <input
  type="date"
  value={customFrom}
