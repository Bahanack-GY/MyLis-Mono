...
const TicketCard = ({
  ...
  <div className={`w-11 h-11 md:w-12 md:h-12  flex items-center justify-center ${STATUS_ICON_STYLES[ticket.status]}`}>
  ...
  className="flex items-center gap-1.5 px-3 py-1.5  text-xs font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] disabled:opacity-50 transition-colors shadow-sm"
  >
...
const TicketDetailModal = ({
  ...
  const rowCls = 'flex items-center justify-between py-3 border-b border-[#e5e8ef] last:border-b-0';
  const labelCls = 'text-sm text-[#8892a4] font-medium';
  ...
  <div className={`w-11 h-11  flex items-center justify-center ${STATUS_ICON_STYLES[ticket.status]}`}>
  <CatIcon size={20} className={STATUS_ICON_TEXT[ticket.status]} />
  </div>
  <div>
  <h2 className="text-lg font-bold text-[#1c2b3a]">{ticket.title}</h2>
  ...
  {ticket.description && (
  <div>
  <h4 className="text-xs font-semibold text-[#8892a4] uppercase tracking-wider mb-2">
  {t('tickets.detail.description')}
  </h4>
  <p className="text-sm text-[#8892a4] leading-relaxed whitespace-pre-wrap bg-[#f8f9fc] p-4">
  {ticket.description}
  </p>
  </div>
  )}
  ...
  <span className="text-sm text-[#b0bac9]">—</span>
  )}
  </div>
  ...
  <div className="w-6 h-6 rounded-full bg-[#f8f9fc] flex items-center justify-center">
  <UserIcon size={12} className="text-[#8892a4]" />
  </div>
  )}
  <span className="text-sm text-[#1c2b3a] font-medium">
  ...
  <span className="text-sm text-[#1c2b3a] font-medium">{ticket.assignedTo.name}</span>
  </div>
  ) : (
  <span className="text-sm text-[#b0bac9] italic">{t('tickets.unassigned')}</span>
  )}
  </div>

  <div className={rowCls}>
  <span className={labelCls}>{t('tickets.detail.createdAt')}</span>
  <span className="text-sm text-[#1c2b3a]">{formatDateFull(ticket.createdAtRaw)}</span>
  </div>

  {ticket.dueDateRaw && (
  <div className={rowCls}>
  <span className={labelCls}>{t('tickets.detail.dueDate')}</span>
  <span className={`text-sm font-medium ${ticket.isOverdue ? 'text-[#283852]' : 'text-[#1c2b3a]'}`}>
  ...
  <span className="text-sm text-[#1c2b3a]">{ticket.closedAt}</span>
  </div>
  )}
  </div>
  </div>
  ...
  {/* Footer */}
  {showAccept && (
  <div className="px-6 py-4 border-t border-[#e5e8ef] flex justify-end shrink-0">
  <button
  onClick={() => { onAccept?.(); onClose(); }}
  disabled={isAccepting}
  className="flex items-center justify-center gap-2 flex-1 py-3 text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] disabled:opacity-50 transition-colors"
  >
