...
function PaymentModal({
  ...
  <h3 className="text-lg font-semibold text-[#1c2b3a]">
  ...
  {/* Invoice selector */}
  <div>
  <label className="block text-sm font-medium text-[#1c2b3a] mb-1">
  ...
  {/* Actions */}
  <div className="flex gap-3 pt-2 border-t border-[#e5e8ef] mt-4 pb-2">
  <button
  type="button"
  onClick={onClose}
...
export default function ClientFollowUp() {
  ...
  className="w-full flex items-center justify-between  border border-gray-200 px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#33cbcc]/40 focus:border-[#33cbcc] transition-colors"
  >
  ...
  exit={{ opacity: 0, y: -4 }}
  className="absolute z-30 mt-1 w-full bg-white border border-gray-200  lg max-h-60 overflow-hidden"
  >
  {/* Search01Icon input */}
  ...
  <>
  {/* Client Info Header */}
  <div className="mx-4 mt-4 border border-gray-200  overflow-hidden">
  ...
  return (
  <div className={`flex items-center gap-2 px-3 py-1  border ${healthStyles.bg} ${healthStyles.text} ${healthStyles.border}`}>
  <HealthIcon size={14} />
  ...
  className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-[#33cbcc]  hover:bg-[#2bb5b6] transition-colors sm"
  >
  <Add01Icon size={16} />
  {t('commercial.followUp.recordPayment')}
  </button>
  ...
  {/* ── Right: Sales Summary ── */}
  <div className="bg-white  sm border border-gray-100 overflow-hidden h-fit">
  {/* Header */}
