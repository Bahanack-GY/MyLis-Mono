...
const StageRoadmap = ({
  ...
  {/* ── Terminal outcomes ── */}
  {isTerminal ? (
  <div className={`flex items-center justify-center gap-2 px-4 py-2.5  text-sm font-bold ${
  currentStage === 'GAGNE'
  ...
  disabled={isPending}
  className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3  bg-[#33cbcc]/10 text-[#33cbcc] text-xs font-semibold hover:bg-[#33cbcc]/20 transition-colors border border-[#33cbcc]/20"
  >
  ...
  disabled={isPending}
  className="flex items-center justify-center gap-1 py-2 px-3  text-xs font-medium text-gray-500 hover:bg-gray-100 transition-colors border border-gray-200"
  >
  <Sad01Icon size={13} />
...
export default function LeadProfileSidebar({
  ...
  const inputCls = 'w-full px-3 py-2 bg-gray-50 border border-gray-200  text-sm focus:ring-2 focus:ring-[#33cbcc]/20 focus:border-[#33cbcc] outline-none transition-all';
  ...
  className="fixed inset-x-0 bottom-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[88vh] bg-white   shadow-2xl z-50 flex flex-col overflow-hidden"
  >
  {isLoading || !lead ? (
  ...
  className="p-2  hover:bg-gray-100 text-gray-400 hover:text-[#33cbcc] transition-colors"
  ...
  <button onClick={onClose} className="p-2  hover:bg-gray-100 text-gray-400 transition-colors">
  ...
  {/* Key metrics row */}
  <div className="flex gap-2">
  <div className="flex-1 bg-gray-50  px-3 py-2 border border-gray-100">
  ...
  {lead.assignedTo && (
  <div className="flex-1 bg-gray-50  px-3 py-2 border border-gray-100 min-w-0">
  ...
  {lead.needs.map(n => (
  <div key={n.id} className="bg-[#283852]/5 border border-gray-100  p-3 flex items-start gap-3">
  ...
  <div className="bg-[#283852]/5 border border-gray-100  p-4">
  ...
  {lead.contacts.map(c => (
  <div key={c.id} className={`border  p-3 space-y-2 ${c.isPrimary ? 'border-[#33cbcc]/30 bg-[#33cbcc]/5' : 'border-gray-100 bg-white'}`}>
  ...
  <div className="bg-white border border-gray-100  p-3 space-y-2">
  <div className="flex items-center gap-2 text-sm font-medium text-gray-800">
  ...
  {lead.activitySector && (
  <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50  px-3 py-2 border border-gray-100">
  ...
  {(lead.city || lead.country) && (
  <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50  px-3 py-2 border border-gray-100 sm:col-span-2">
  ...
  {/* Competitor */}
  {lead.competitor && (
  <div className="bg-[#283852]/5 border border-gray-100  p-3">
  ...
  className="w-full flex items-center justify-center gap-2 px-4 py-3  border border-dashed border-[#33cbcc] text-[#33cbcc] font-medium text-sm hover:bg-[#33cbcc]/5 transition-colors"
  >
  <Add01Icon size={15} />
  ...
  className="bg-gray-50 border border-gray-100  p-4 space-y-3 overflow-hidden"
  >
  ...
  <button onClick={() => setShowActivityForm(false)} className="px-3 py-1.5 text-xs text-gray-600 bg-white border border-gray-200  hover:bg-gray-50 transition-colors">
  {t('common.cancel', 'Annuler')}
  </button>
  <button
  onClick={handleLogActivity}
  disabled={!actForm.description.trim() || createActivity.isPending}
  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-[#33cbcc]  hover:bg-[#2bb5b6] disabled:opacity-50 transition-colors"
  >
  ...
  <div key={act.id} className="flex gap-3 p-3 bg-white border border-gray-100  hover:border-[#33cbcc]/30 transition-colors">
  <div className="w-9 h-9  bg-gray-50 flex items-center justify-center shrink-0 border border-gray-100 text-gray-500">
  ...
  <span className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5  shrink-0 ml-2">
  {new Date(act.date).toLocaleDateString('fr-FR')}
  </span>
  </div>
  <p className="text-sm text-gray-600 leading-relaxed">{act.description}</p>
  {act.result && <p className="mt-1.5 text-xs text-gray-500 bg-gray-50 p-2  border border-gray-100"><span className="font-semibold">Résultat:</span> {act.result}</p>}
  </div>
  </div>
  ))}
  </div>
  ...
  return (
  <div key={task.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 ">
  <div className="flex items-center gap-2.5 min-w-0">
  <span className={`px-2 py-0.5  text-[10px] font-bold uppercase tracking-wide shrink-0 ${stateColors[task.state] || 'bg-gray-100 text-gray-600'}`}>
  {task.state}
  </span>
  <span className="text-sm font-medium text-gray-800 truncate">{task.title}</span>
  </div>
  {task.dueDate && (
  <div className="flex items-center gap-1 text-xs text-gray-400 bg-gray-50 px-2 py-1  border border-gray-100 shrink-0 ml-2">
  <Clock01Icon size={11} />
  ...
  className="w-full flex items-center justify-center gap-2 px-4 py-2.5  text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] transition-colors shadow-sm shadow-[#33cbcc]/20"
  >
  <Tick01Icon size={16} />
