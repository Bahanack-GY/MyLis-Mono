...
const PayslipDetailModal = ({
  ...
  >
  {/* Header */}
  <div className="shrink-0 bg-white border-b border-[#e5e8ef] px-6 py-4 flex items-center justify-between">
  <div>
  <h3 className="text-lg font-bold text-[#1c2b3a]">Fiche de Paie</h3>
  <p className="text-sm text-[#8892a4]">{MONTHS[run.month - 1]} {run.year} - {empName}</p>
  </div>
  <div className="flex items-center gap-2">
  <button
  onClick={onDownload}
  disabled={downloading}
  className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] transition-colors disabled:opacity-60"
  >
  ...
  {/* Employee Info */}
  <div className="bg-[#f8f9fc]  p-4 grid grid-cols-2 gap-3">
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider">Employe</p>
  <p className="text-sm font-semibold text-[#1c2b3a] mt-0.5">{empName}</p>
  </div>
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider">Departement</p>
  <p className="text-sm text-[#1c2b3a] mt-0.5">{emp.department?.name || '-'}</p>
  </div>
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider">Periode</p>
  <p className="text-sm text-[#1c2b3a] mt-0.5">{MONTHS[run.month - 1]} {run.year}</p>
  </div>
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider">Date de paiement</p>
  <p className="text-sm text-[#1c2b3a] mt-0.5">{run.paidAt ? formatDate(run.paidAt) : '-'}</p>
  </div>
  </div>

  {/* Gross Salary */}
  <div className="flex justify-between items-center px-1">
  <span className="text-sm font-semibold text-[#1c2b3a]">Salaire Brut</span>
  <span className="text-base font-bold text-[#1c2b3a]">{formatXAF(Number(payslip.grossSalary) || 0)}</span>
  </div>

  {/* Deductions table */}
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider mb-2">Retenues salariales</p>
  ...
  <span className="text-[#1c2b3a]">{row.label}</span>
  {row.rate && <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#e5e8ef] text-[#8892a4] font-medium">{row.rate}</span>}
  </div>
  <span className="font-medium text-[#283852]">- {formatXAF(row.amount)}</span>
  </div>
  ))}
  <div className="flex items-center justify-between px-4 py-2.5 bg-[#f8f9fc] border-t border-[#e5e8ef]">
  <span className="text-sm font-bold text-[#1c2b3a]">Total Retenues</span>
  ...
  {/* Employer charges */}
  <div>
  <p className="text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider mb-2">Charges patronales</p>
  <div className="bg-[#283852]/5  overflow-hidden">
  <div className="flex items-center justify-between px-4 py-2.5 text-sm">
  <span className="text-[#1c2b3a]">CNPS Employeur</span>
  <span className="font-medium text-[#283852]">{formatXAF(Number(payslip.cnpsEmployer) || 0)}</span>
  </div>
  <div className="flex items-center justify-between px-4 py-2.5 bg-[#283852]/10 border-t border-[#e5e8ef]">
  <span className="text-sm font-bold text-[#1c2b3a]">Total Charges Patronales</span>
  ...
  {/* Net salary box */}
  <div className="bg-gradient-to-r from-[#33cbcc] to-[#33cbcc]  p-5 flex items-center justify-between">
  <div>
...
const PayslipCard = ({
  ...
  className="bg-white  border border-gray-100  transition-all duration-300 group"
  >
  <div className="p-5">
  {/* Period header */}
  ...
  <button
  onClick={onView}
  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5  text-sm font-medium text-gray-600 bg-gray-50 hover:bg-gray-100 transition-colors"
  >
  <ViewIcon size={14} />
  Details
  </button>
  ...
  onClick={onDownload}
  disabled={downloading}
  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5  text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] transition-colors disabled:opacity-60 shadow-sm shadow-[#33cbcc]/20"
  >
  {downloading ? <Loading02Icon size={14} className="animate-spin" /> : <Download01Icon size={14} />}
  Telecharger
  </button>
...
export default function MyPayslips() {
  ...
  className="border border-gray-100  overflow-hidden transition-colors duration-200 hover:border-[#33cbcc]/50"
  >
