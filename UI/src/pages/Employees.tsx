...
const CreateEmployeeModal = ({ onClose, initialUserType = 'employee', hodDepartmentId }: { onClose: () => void; initialUserType?: UserType; hodDepartmentId?: string }) => {
  ...
  const docInputCls = 'flex-1 bg-[#f8f9fc] border border-[#e5e8ef] px-3 py-2 text-xs text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors';
  ...
  <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-[#e5e8ef]">
  {avatarPreview ? (
  <img src={avatarPreview} alt="" className="w-full h-full object-cover" />
  ) : (
  <div className="w-full h-full bg-[#f8f9fc] flex items-center justify-center">
  <UserIcon size={28} className="text-[#8892a4]" />
  </div>
  )}
  </div>
  <button
  ...
  onClick={() => avatarInputRef.current?.click()}
  className="flex-1 border-2 border-dashed border-[#e5e8ef]  p-4 text-center cursor-pointer hover:border-[#33cbcc]/40 transition-colors"
  >
  <Upload01Icon size={20} className="mx-auto text-[#8892a4] mb-1" />
  <p className="text-xs text-[#8892a4]">{t('employees.edit.dragOrClick')}</p>
  <p className="text-[10px] text-[#8892a4] mt-0.5">{t('employees.edit.maxSize')}</p>
  </div>
  <input
  ref={avatarInputRef}
  type="file"
  ...
  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8892a4] hover:text-[#1c2b3a]"
  >
  ...
  <p className="text-xs text-[#8892a4] mb-3">{t('employees.create.skillsHint')}</p>
  ...
  className="flex-1 bg-[#f8f9fc] border border-[#e5e8ef] px-3 py-2 text-xs text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors"
  />
  <button
  type="button"
  ...
  : 'bg-[#f8f9fc] text-[#8892a4] cursor-not-allowed'
  }`}
  >
  <Add01Icon size={14} />
  </button>
  ...
  <p className="text-xs text-[#8892a4] mb-3">{t('employees.create.educationDocsHint')}</p>
  {form.educationDocs.length > 0 && (
  <div className="space-y-2">
  {form.educationDocs.map((doc, i) => (
  <div key={i} className="bg-[#f8f9fc] border border-[#e5e8ef]  p-2.5 space-y-2">
  ...
  className="bg-[#f8f9fc] border border-[#e5e8ef] px-3 py-2 text-xs text-[#1c2b3a] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors"
  >
  ...
  <div className={`flex items-center gap-2 px-3 py-1.5  text-xs font-medium transition-all max-w-[220px] ${
  doc.file
  ? 'bg-[#33cbcc]/10 text-[#33cbcc] border border-[#33cbcc]/20'
  : 'bg-[#f8f9fc] text-[#8892a4] border border-[#e5e8ef] hover:border-[#33cbcc]/30'
  }`}>
  ...
  <p className="text-xs text-[#8892a4] mb-3">{t('employees.create.recruitmentDocsHint')}</p>
...
const PersonNode = ({ emp, badge, onClick }: { emp: any; badge: string; onClick: () => void }) => (
  <div
  onClick={onClick}
  className="inline-block bg-white  border border-gray-100 shadow-md overflow-hidden w-48 text-center cursor-pointer hover:border-[#283852]/30 hover:shadow-lg transition-all"
  >
...
const CompanyNode = () => (
  <div className="inline-flex items-center gap-2.5 bg-gradient-to-r from-[#283852] to-[#1e2a3d] text-white px-5 py-3  shadow-lg cursor-default">
  <div className="w-7 h-7  bg-[#33cbcc]/20 flex items-center justify-center shrink-0">
...
const HodNode = ({ dept, navigate, showMembers = false }: { dept: any; navigate: (path: string) => void; showMembers?: boolean }) => {
  ...
  className={`bg-white  border border-gray-100 shadow-md overflow-hidden w-52 ${head ? 'cursor-pointer hover:border-[#33cbcc]/40 hover:shadow-lg transition-all' : ''}`}
  >
  {/* Dept label */}
  ...
  className="bg-white  border border-gray-100 shadow-sm px-3 py-2 flex items-center gap-2.5 cursor-pointer hover:border-[#33cbcc]/30 hover:shadow-md transition-all"
  >
  <img
  src={orgAvatar(m)}
...
const MemberNode = ({ emp, navigate }: { emp: any; navigate: (path: string) => void }) => (
  ...
  className="inline-block bg-white  border border-gray-100 shadow-sm overflow-hidden w-44 text-center px-4 py-4 flex flex-col items-center gap-2.5 cursor-pointer hover:border-[#33cbcc]/30 hover:shadow-md transition-all"
  >
  <img
  src={orgAvatar(emp)}
...
const Employees = () => {
  ...
  {/* Avatar stack + count */}
  {employees.length > 0 && (
  <div className="flex items-center gap-2 border border-[#e5e8ef] bg-white  px-3 py-2">
  ...
  {/* View toggle */}
  <div className="flex items-center border border-[#e5e8ef] bg-white  p-1">
  {(['org','grid','list'] as const).map(mode => (
  <button key={mode} onClick={() => setViewMode(mode)}
  className={`p-2  transition-all duration-150 ${viewMode === mode ? 'bg-[#283852] text-white' : 'text-[#8892a4] hover:text-[#1c2b3a]'}`}
  ...
  className="flex items-center gap-2 bg-[#33cbcc] text-white px-5 py-2.5  text-sm font-semibold"
  >
  ...
  className="w-full bg-white border border-[#e5e8ef]  py-3 pl-10 pr-8 text-sm text-[#1c2b3a] appearance-none cursor-pointer focus:outline-none focus:border-[#33cbcc] transition-colors"
  >
  ...
  <div className="bg-[#283852]  overflow-hidden">
  ...
  <div className="mt-3 px-3 py-1 bg-[#33cbcc]/15 text-[#33cbcc] text-xs font-bold  border border-[#33cbcc]/20">
  ...
  className="flex items-center gap-3  px-4 py-3 border border-white/6 hover:bg-white/6 transition-colors"
  >
  ...
  className="group bg-white border border-[#e5e8ef]  overflow-hidden cursor-pointer hover:border-[#33cbcc] transition-colors duration-200"
  >
  {/* Color accent bar */}
  ...
  <span className="mt-3 text-[10px] font-semibold px-2.5 py-1 " style={{ backgroundColor: accentColor + '18', color: accentColor }}>
  {employee.departmentName}
  </span>
  )}
  </div>
  ...
  {employee.departmentName && (
  <span className="text-[11px] font-semibold px-2.5 py-1  hidden md:inline-flex items-center gap-1.5 shrink-0"
  ...
  className="flex items-center gap-2.5 px-4 py-2.5 bg-white border border-[#e5e8ef]  text-sm font-semibold text-[#8892a4] hover:border-[#283852]/30 hover:text-[#283852] transition-colors mb-4"
  >
  <UserAdd01Icon size={14} className="rotate-45 shrink-0" />
  {t('employees.dismissed', 'Dismissed / Suspended')}
  <span className="px-2 py-0.5 bg-[#f0f2f5] text-[#8892a4]  text-xs font-bold">
  ...
  onClick={() => navigate(`/employees/${employee.id}`)}
  className="group bg-white border border-[#e5e8ef]  overflow-hidden cursor-pointer opacity-55 grayscale hover:opacity-70 transition-all"
  >
  ...
  <span className="mt-3 text-[10px] font-bold px-2.5 py-1  bg-[#f3f4f6] text-[#9ca3af] uppercase tracking-wide">
  ...
  <span className="text-[10px] font-bold px-2.5 py-1  bg-[#f3f4f6] text-[#9ca3af] uppercase tracking-wide shrink-0">
  {t('employees.dismissedBadge', 'Dismissed')}
  </span>
