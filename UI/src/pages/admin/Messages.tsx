...
const DemandCardBubble = ({
  ...
  onSuccess: () => {
  setActionTaken('VALIDATED');
  // Notify all channel members (including user) of the status change
  socket?.emit('demand:statusUpdate', { demandId: data.demandId, status: 'VALIDATED' });
  },
  });
  };
  ...
  // Notify all channel members (including user) of the status change
  socket?.emit('demand:statusUpdate', { demandId: data.demandId, status: 'REJECTED' });
  },
  });
  };
  ...
  className="flex-1 flex items-center justify-center gap-1.5 py-2  text-xs font-semibold text-white/70 bg-white/10 hover:bg-white/20 transition-colors"
  >
  ...
  className="flex-1 flex items-center justify-center gap-1.5 py-2  text-xs font-semibold text-[#33cbcc] bg-[#33cbcc]/20 hover:bg-[#33cbcc]/30 transition-colors"
  >
  ...
  rows={2}
  className="w-full bg-white/10  px-3 py-2 text-xs text-white placeholder-white/40 focus:outline-none focus:ring-1 focus:ring-[#33cbcc]/30 resize-none"
  autoFocus
  />
  <div className="flex gap-2">
  <button
  onClick={() => setShowRejectInput(false)}
  className="flex-1 py-1.5  text-xs font-medium text-white/60 bg-white/10 hover:bg-white/15 transition-colors"
  >
  {t('demands.cancel')}
  </button>
  <button
  ...
  className="flex-1 flex items-center justify-center gap-1 py-1.5  text-xs font-semibold text-white/70 bg-white/10 hover:bg-white/20 transition-colors"
  >
...
const AttachmentRenderer = ({
  ...
  exit={{ scale: 0.9, opacity: 0 }}
  onClick={e => e.stopPropagation()}
  className="relative w-full max-w-4xl h-[85vh] bg-white  overflow-hidden flex flex-col"
  >
  ...
  <button onClick={() => setPdfPreview(null)} className="p-1.5  hover:bg-gray-100 text-gray-500">
...
const Messages = () => {
  ...
  className="md:hidden p-1.5  text-gray-400 hover:bg-gray-100 transition-colors shrink-0"
  >
