...
const UploadDocumentModal = ({ onClose }: { onClose: () => void }) => {
  ...
  <p className="text-sm font-medium text-[#1c2b3a]">{form.file.name}</p>
  ...
  {/* Visibility Settings */}
  <div className="bg-[#283852]/10  p-4 space-y-4">
  ...
  <div>
  <label className="text-[10px] font-semibold text-[#8892a4] mb-1.5 block">
  ...
  className="rounded"
  />
  <span className="text-[#1c2b3a]">{dept.name}</span>
  </label>
  ))}
  </div>
  </div>
  ...
  className="rounded"
  />
  <span className="text-[#1c2b3a]">{emp.firstName} {emp.lastName}</span>
  </label>
  ))}
  </div>
  </div>
...
const Documents = () => {
  ...
  'CONTRACT': 'Contract', 'ID': 'Notes', 'DIPLOMA': 'Education',
  'RECRUITMENT': 'Recruitment', 'OTHER': 'Technical',
  };
  ...
  className={`group p-4 border border-[#e5e8ef] hover:border-[#c8cdd8] hover:bg-[#f8f9fc] transition-all ${doc.filePath ? 'cursor-pointer' : ''}`}
  >
  ...
  <span className="text-xs text-[#8892a4]">{doc.size}</span>
  <span className="text-xs text-[#8892a4]">•</span>
  <span className="text-xs text-[#8892a4]">{doc.date}</span>
  <span className="text-xs text-[#8892a4]">•</span>
  ...
  <span className="text-xs text-[#8892a4] truncate max-w-25">{doc.uploader.name}</span>
  </div>
  </div>
  </div>
  ...
  className="p-2 hover:bg-[#283852]/10 text-[#8892a4] hover:text-[#283852] transition-colors"
  title="Delete"
  >
