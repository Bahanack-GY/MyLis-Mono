...
const KpiCard = ({ label, value, sub, icon: Icon, accent = TEAL, index, onClick }: KpiCardProps) => (
  ...
  animate="visible"
  onClick={onClick}
  className={`border border-gray-100  overflow-hidden ${onClick ? 'cursor-pointer' : ''}`}
  >
...
const DateFilter = ({ preset, setPreset, draftFrom, setDraftFrom, draftTo, setDraftTo, onApply }: DateFilterProps) => {
  ...
  <button
  onClick={() => setOpen(v => !v)}
  className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200  text-sm text-gray-700 hover:border-[#33cbcc]/40 transition-colors shadow-sm"
  >
  ...
  className="absolute right-0 top-full mt-2 bg-white  border border-gray-100 shadow-xl z-50 min-w-[210px] overflow-hidden"
  >
  {presets.map(p => (
  <button
  key={p}
  ...
  className="w-full mt-1 px-3 py-1.5 border border-gray-200  text-sm text-gray-700 focus:outline-none focus:border-[#33cbcc]"
  />
  </div>
  <div>
  ...
  className="w-full px-3 py-2 bg-[#33cbcc] text-white text-sm font-semibold  hover:bg-[#2bb5b6] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
  >
  Appliquer
  </button>
  </div>
...
const CeoDashboard = () => {
  ...
  <div className="w-11 h-11  bg-[#283852] flex items-center justify-center shrink-0">
  ...
  className="px-4 py-2.5 bg-white border border-gray-200  text-sm text-gray-700 focus:ring-2 focus:ring-[#33cbcc]/20 focus:border-[#33cbcc] outline-none appearance-none cursor-pointer shadow-sm"
  >
  ...
  className="bg-white  p-5 space-y-3 cursor-pointer hover:shadow-md transition-shadow"
  >
  ...
  <div className="mt-4 flex items-center justify-between px-4 py-3 bg-[#283852]/5 ">
  ...
  className="flex flex-col items-center gap-2.5 bg-white  p-5 hover:shadow-md transition-all group text-center"
  >
  <div
