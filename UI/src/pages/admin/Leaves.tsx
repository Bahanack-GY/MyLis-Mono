...
import { Calendar01Icon, Tick01Icon, Clock01Icon, CancelCircleIcon, Cancel01Icon, Search01Icon, UserIcon, Loading02Icon, Edit01Icon } from 'hugeicons-react';
import { useLeaves, useApproveLeave, useRejectLeave, useLeaveBalance, useUpdateLeaveBalance } from '../../api/leaves/hooks';
import type { LeaveRequest, LeaveStatus } from '../../api/leaves/types';
...
/* ─── Balance Edit Modal ────────────────────────────────────── */
const BalanceModal = ({ employeeId, onClose }: { employeeId: string; onClose: () => void }) => {
  ...
  const handleSave = async () => {
    ...
    await updateBalance.mutateAsync({ employeeId, year, totalDays: days });
    onClose();
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <h3 className="text-lg font-semibold text-[#283852] mb-1">Modifier le solde</h3>
        {balance && (
          <p className="text-sm text-gray-500 mb-4">
            Actuel: {balance.totalDays}j total · {balance.usedDays}j utilisés · {balance.remainingDays}j restants
          </p>
        )}
        <label className="block text-sm text-gray-600 mb-1">Nouveau total (jours)</label>
        <input
          type="number"
          min={0}
          step={0.5}
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#283852]/20"
          placeholder={balance ? String(balance.totalDays) : '18'}
          value={totalDays}
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button
            onClick={handleSave}
            disabled={!totalDays || updateBalance.isPending}
            className="flex-1 px-4 py-2 text-sm rounded-xl bg-[#283852] text-white hover:bg-[#283852]/90 disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {updateBalance.isPending && <Loading02Icon size={14} className="animate-spin" />}
            Enregistrer
          </button>
        </div>
      </motion.div>
...
/* ─── Detail Modal ──────────────────────────────────────────── */
const DetailModal = ({ request, onClose }: { request: LeaveRequest; onClose: () => void }) => {
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <h3 className="text-lg font-semibold text-[#283852]">{TYPE_LABELS[request.type] || request.type}</h3>
              <div className="flex items-center gap-2 mt-1">
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_BG[request.status]}`}>
                  <StatusIcon size={12} />
          ...
          <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-xl">
            ...
            <button
              ...
              className="ml-auto text-xs text-[#33cbcc] flex items-center gap-1 hover:underline"
            >
              <Edit01Icon size={12} /> Solde
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="p-3 bg-gray-50 rounded-xl">
              ...
              <p className="text-sm font-medium text-[#283852]">{formatDate(request.startDate)}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-xl">
              <p className="text-xs text-gray-400 mb-0.5">Fin</p>
              <p className="text-sm font-medium text-[#283852]">{formatDate(request.endDate)}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-xl">
              <p className="text-xs text-gray-400 mb-0.5">Durée</p>
              <p className="text-sm font-medium text-[#283852]">{request.numberOfDays} jour{request.numberOfDays > 1 ? 's' : ''}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-xl">
              <p className="text-xs text-gray-400 mb-0.5">Soumis le</p>
              <p className="text-sm font-medium text-[#283852]">{formatDate(request.createdAt)}</p>
            </div>
          </div>
      ...
      <AnimatePresence>
        ...
        {showBalance && request.employee && (
          <BalanceModal employeeId={request.employee.id} onClose={() => setShowBalance(false)} />
        )}
      </AnimatePresence>
    </>
  );
...
export default function AdminLeaves() {
  ...
  const { data: requests = [], isLoading } = useLeaves(
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#283852]">Demandes de congé</h1>
        <p className="text-sm text-gray-500 mt-1">Gérez et validez les demandes de congé des employés</p>
      </div>
              ...
              {filtered.map(req => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-600">{TYPE_LABELS[req.type] || req.type}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">
                      {formatDate(req.startDate)} → {formatDate(req.endDate)}
                    </td>
                    <td className="px-5 py-3.5 text-sm font-medium text-[#283852]">
                      {req.numberOfDays}j
                    </td>
                    <td className="px-5 py-3.5">
