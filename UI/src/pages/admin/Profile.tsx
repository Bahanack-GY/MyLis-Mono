...
const EditProfileModal = ({
  ...
  <button className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] transition-colors">
...
const ChangePasswordModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const inputCls = 'w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 pr-10 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors';
  const labelCls = 'flex items-center gap-1.5 text-[10px] font-semibold text-[#8892a4] uppercase tracking-wider mb-1.5';
  ...
  <h2 className="text-lg font-bold text-[#1c2b3a]">{t('profile.changePassword', 'Change Password')}</h2>
  </div>
  <button onClick={onClose} className="p-2 hover:bg-[#f8f9fc] text-[#8892a4] hover:text-[#1c2b3a] transition-colors">
  <Cancel01Icon size={18} />
  </button>
  ...
  <div className="flex-1 h-2 bg-[#f8f9fc] rounded-full overflow-hidden">
  <div
  className={`h-full transition-all duration-300 ${strengthColor}`}
  style={{ width: `${(strengthScore / 4) * 100}%` }}
  />
  </div>
  <span className="text-xs font-medium text-[#8892a4]">{strengthLabel}</span>
  </div>
  {passwordStrength && passwordStrength.length > 0 && (
  <div className="space-y-1">
  {passwordStrength.map((issue, i) => (
  <p key={i} className="text-xs text-[#8892a4] flex items-center gap-1">
  <span className="w-1 h-1 rounded-full bg-[#b0bac9]" />
  {issue}
  </p>
  ))}
  </div>
...
const Profile = () => {
  ...
  <div className="w-28 h-28  border-4 border-white shadow-xl overflow-hidden bg-white">
  {profile.avatar ? (
  ...
  className="flex items-center gap-2 bg-gray-100 text-gray-700 px-5 py-2.5  text-sm font-semibold hover:bg-gray-200 transition-colors"
  >
