...
function FileChip({ file, onRemove }: { file: JustificationFile; onRemove?: () => void }) {
  ...
  return (
  <div className="flex items-center gap-2 bg-gray-50 border border-gray-200  px-3 py-2 group">
  {isPdf
  ? <File01Icon size={14} className="text-[#283852] shrink-0" />
  : <img src={url} alt={file.originalName} className="w-6 h-6  object-cover border border-gray-100 shrink-0" />
  }
...
function PendingFileChip({ file, onRemove }: { file: File; onRemove: () => void }) {
  ...
  return (
  <div className="flex items-center gap-2 bg-[#33cbcc]/5 border border-[#33cbcc]/30  px-3 py-2">
  {isPdf
  ? <File01Icon size={14} className="text-[#33cbcc] shrink-0" />
  : preview && <img src={preview} alt={file.name} className="w-6 h-6  object-cover shrink-0" />
  }
...
export default function FundMovements() {
  ...
  initial="hidden"
  animate="visible"
  className={`bg-white  border ${card.border} shadow-md p-6`}
  >
  ...
  className="text-[10px] bg-red-500 text-white px-2 py-1  font-semibold hover:bg-red-600 transition-colors"
  >
  Oui
  </button>
  <button
  onClick={() => setConfirmDelete(null)}
  className="text-[10px] bg-gray-200 text-gray-600 px-2 py-1  font-semibold hover:bg-gray-300 transition-colors"
  >
  Non
  </button>
  </div>
  ...
  className="text-gray-400 hover:text-red-500 transition-colors p-1  hover:bg-red-50"
  >
  ...
  <div>
  <h2 className="text-xl font-bold text-[#1c2b3a]">Nouveau mouvement</h2>
  ...
  className="text-[#8892a4] hover:text-[#1c2b3a] transition-colors p-1  hover:bg-[#f8f9fc]"
  >
  ...
  {/* Type toggle */}
  <div>
  <label className="text-xs font-semibold text-[#8892a4] uppercase tracking-wider mb-2 block">
  Type de mouvement
  </label>
  ...
  {isAccountant && (
  <div>
  <label className="text-xs font-semibold text-[#8892a4] uppercase tracking-wider mb-1.5 block">
  CEO concerné
  </label>
  <select
  ...
  <div>
  <label className="text-xs font-semibold text-[#8892a4] uppercase tracking-wider mb-1.5 block">
  Justificatif(s) <span className="text-[#8892a4] font-normal normal-case">(image, PDF)</span>
  </label>
  <div
  onDragOver={e => e.preventDefault()}
  onDrop={handleFileDrop}
  onClick={() => fileInputRef.current?.click()}
  className="border-2 border-dashed border-[#e5e8ef] p-4 flex flex-col items-center gap-2 cursor-pointer hover:border-[#33cbcc]/50 hover:bg-[#33cbcc]/5 transition-all"
  >
  <Upload01Icon size={20} className="text-[#b0bac9]" />
  <p className="text-xs text-[#8892a4]">Glisser-déposer ou cliquer pour ajouter</p>
  <input
  ref={fileInputRef}
  type="file"
  multiple
  ...
  >
  <p className="text-xs text-[#8892a4] font-semibold mb-2 uppercase tracking-wider">
  ...
  className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white transition-all ${
  form.type === 'APPORT'
  ? 'bg-[#33cbcc] hover:bg-[#2bb5b6]'
  : 'bg-[#283852] hover:bg-[#1e2a3d]'
  }`}
  >
