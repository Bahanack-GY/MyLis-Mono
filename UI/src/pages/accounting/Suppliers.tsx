...
function SupplierModal({ supplier, onClose }: { supplier: Supplier | null; onClose: () => void }) {
  ...
  <div className="flex items-center justify-between p-6 border-b border-[#e5e8ef]">
  <h2 className="text-lg font-semibold text-[#1c2b3a]">{supplier ? 'Modifier fournisseur' : 'Nouveau fournisseur'}</h2>
  <button onClick={onClose} className="p-2 hover:bg-[#f8f9fc]"><Cancel01Icon size={18} /></button>
  </div>
  <div className="p-6 space-y-4 flex-1 overflow-y-auto">
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Nom *</label>
  ...
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Email</label>
  <input value={form.email} onChange={e => set('email', e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Téléphone</label>
  <input value={form.phone} onChange={e => set('phone', e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Adresse</label>
  <input value={form.address} onChange={e => set('address', e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  <div className="grid grid-cols-2 gap-3">
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">RCCM</label>
  <input value={form.rccm} onChange={e => set('rccm', e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">NIU</label>
  <input value={form.niu} onChange={e => set('niu', e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Délai de paiement (jours)</label>
  <input type="number" value={form.paymentTermsDays} onChange={e => set('paymentTermsDays', +e.target.value)}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Notes</label>
  <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={2}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] placeholder-[#b0bac9] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  </div>
  <div className="flex gap-3 p-6 border-t border-[#e5e8ef]">
  ...
  className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-[#33cbcc] hover:bg-[#2bb5b6] disabled:opacity-50">
...
function InvoiceFormModal({ invoice, suppliers, onClose }: {
  ...
  className="bg-white w-full max-w-md h-full flex flex-col border-l border-[#e5e8ef]">
  <div className="flex items-center justify-between p-6 border-b border-[#e5e8ef]">
  <h2 className="text-lg font-semibold text-[#1c2b3a]">{invoice ? 'Modifier la facture' : 'Nouvelle facture fournisseur'}</h2>
  <button onClick={onClose} className="p-2 hover:bg-[#f8f9fc]"><Cancel01Icon size={18} /></button>
  </div>
  ...
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Fournisseur *</label>
  ...
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Département</label>
  ...
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">N° Facture (auto si vide)</label>
  ...
  <div />
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Date facture *</label>
  <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
  className="w-full bg-[#f8f9fc] border border-[#e5e8ef] px-4 py-3 text-sm text-[#1c2b3a] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  </div>
  <div>
  <label className="block text-sm font-medium mb-1 text-[#1c2b3a]">Date d'échéance *</label>
  ...
  <h3 className="font-medium text-sm text-[#1c2b3a]">Lignes</h3>
  ...
  <div className="border border-[#e5e8ef] overflow-hidden">
  <table className="w-full text-sm">
  <thead className="bg-[#f8f9fc]">
  <tr>
  <th className="text-left px-3 py-2 font-medium text-[#8892a4]">Description</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4] w-20">Qté</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4] w-28">P.U. HT</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4] w-20">TVA %</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4] w-28">Total TTC</th>
  <th className="w-10" />
  </tr>
  </thead>
  <tbody>
  {computedItems.map((item, idx) => (
  <tr key={idx} className="border-t border-[#e5e8ef]">
  <td className="px-3 py-2">
  <input value={item.description} onChange={e => setItem(idx, 'description', e.target.value)}
  placeholder="Description..."
  className="w-full focus:outline-none text-sm text-[#1c2b3a]" />
  </td>
  <td className="px-3 py-2">
  <input type="number" value={item.quantity} onChange={e => setItem(idx, 'quantity', +e.target.value)}
  className="w-full text-right focus:outline-none text-sm text-[#1c2b3a]" />
  </td>
  <td className="px-3 py-2">
  ...
  <td className="px-3 py-2 text-right font-medium text-[#1c2b3a]">{fmt(item.totalTTC)}</td>
  <td className="px-3 py-2">
  {items.length > 1 && (
  <button onClick={() => removeItem(idx)} className="text-[#8892a4] hover:text-[#283852]">
  ...
  <span className="text-[#8892a4]">Sous-total HT: <span className="font-medium text-[#1c2b3a]">{fmt(totalHT)} FCFA</span></span>
  <span className="text-[#8892a4]">TVA: <span className="font-medium text-[#1c2b3a]">{fmt(taxAmount)} FCFA</span></span>
  <span className="text-[#8892a4] font-semibold">Total TTC: <span className="text-[#283852] text-base font-bold">{fmt(totalTTC)} FCFA</span></span>
  </div>
  </div>
...
function InvoiceDetailModal({ invoice, onClose }: { invoice: SupplierInvoice; onClose: () => void }) {
  ...
  <h2 className="text-lg font-semibold text-[#1c2b3a]">{invoice.invoiceNumber}</h2>
  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${cfg.color}`}>
  <cfg.icon size={12} /> {cfg.label}
  </span>
  </div>
  <p className="text-sm text-[#8892a4] mt-0.5">{invoice.supplier.name}</p>
  </div>
  <button onClick={onClose} className="p-2 hover:bg-[#f8f9fc]"><Cancel01Icon size={18} /></button>
  </div>
  <div className="overflow-y-auto flex-1 p-6 space-y-5">
  <div className="grid grid-cols-3 gap-4 text-sm">
  <div><span className="text-[#8892a4]">Date:</span> <span className="font-medium ml-1 text-[#1c2b3a]">{fmtDate(invoice.date)}</span></div>
  <div><span className="text-[#8892a4]">Échéance:</span> <span className="font-medium ml-1 text-[#1c2b3a]">{fmtDate(invoice.dueDate)}</span></div>
  {invoice.department && <div><span className="text-[#8892a4]">Département:</span> <span className="font-medium ml-1 text-[#1c2b3a]">{invoice.department.name}</span></div>}
  {invoice.paidAt && <div><span className="text-[#8892a4]">Payée le:</span> <span className="font-medium ml-1 text-[#33cbcc]">{fmtDate(invoice.paidAt)}</span></div>}
  </div>
  ...
  <tr>
  <th className="text-left px-3 py-2 font-medium text-[#8892a4]">Description</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4]">Qté</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4]">P.U. HT</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4]">TVA</th>
  <th className="text-right px-3 py-2 font-medium text-[#8892a4]">Total TTC</th>
  </tr>
  </thead>
  <tbody>
  {invoice.items.map((item, idx) => (
  <tr key={idx} className="border-t border-[#e5e8ef]">
  <td className="px-3 py-2 text-[#1c2b3a]">{item.description}</td>
  <td className="px-3 py-2 text-right text-[#1c2b3a]">{item.quantity}</td>
  <td className="px-3 py-2 text-right text-[#1c2b3a]">{fmt(item.unitPrice)}</td>
  <td className="px-3 py-2 text-right text-[#1c2b3a]">{item.taxRate}%</td>
  <td className="px-3 py-2 text-right font-medium text-[#1c2b3a]">{fmt(item.totalTTC ?? 0)}</td>
  </tr>
  ))}
  </tbody>
  </table>
  ...
  <span className="text-[#8892a4]">HT: <span className="font-medium text-[#1c2b3a]">{fmt(Number(invoice.totalHT))} FCFA</span></span>
  <span className="text-[#8892a4]">TVA: <span className="font-medium text-[#1c2b3a]">{fmt(Number(invoice.taxAmount))} FCFA</span></span>
  <span className="font-bold text-[#283852] text-base">{fmt(Number(invoice.totalTTC))} FCFA</span>
  </div>

  {invoice.notes && <p className="text-sm text-[#8892a4] bg-[#f8f9fc] p-3">{invoice.notes}</p>}

  {/* Pay date picker */}
  <AnimatePresence>
  {showPayDate && (
  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
  className="bg-[#283852]/10 p-4 flex items-center gap-3">
  ...
  className="bg-[#f8f9fc] border border-[#e5e8ef] px-2 py-1 text-sm text-[#1c2b3a] focus:outline-none focus:border-[#33cbcc] focus:bg-white transition-colors" />
  <button onClick={() => pay.mutate()} disabled={pay.isPending}
  className="ml-auto px-3 py-1.5 bg-[#33cbcc] text-white text-sm hover:bg-[#2bb5b6] disabled:opacity-50">
  {pay.isPending ? '...' : 'Confirmer paiement'}
  </button>
  <button onClick={() => setShowPayDate(false)} className="text-[#8892a4] hover:text-[#1c2b3a]"><Cancel01Icon size={16} /></button>
  </motion.div>
  )}
  </AnimatePresence>
  </div>
  <div className="flex items-center justify-between gap-3 p-6 border-t border-[#e5e8ef]">
  <div className="flex gap-2">
  {invoice.status === 'DRAFT' && (
  <button onClick={() => cancel.mutate()} disabled={cancel.isPending}
  className="px-3 py-1.5 text-sm border border-[#e5e8ef] text-[#283852] hover:bg-[#283852]/10 disabled:opacity-50">
  Annuler
  </button>
  )}
  ...
  className="px-4 py-2 text-sm bg-[#33cbcc] text-white hover:bg-[#2bb5b6] disabled:opacity-50">
  {validate.isPending ? '...' : 'Valider'}
  </button>
  )}
  {invoice.status === 'VALIDATED' && !showPayDate && (
  <button onClick={() => setShowPayDate(true)}
  className="px-4 py-2 text-sm bg-[#33cbcc] text-white hover:bg-[#2bb5b6]">
  Payer
  </button>
  )}
  </div>
...
export default function Suppliers() {
  ...
  const { data: invoicesRaw } = useInvoices({ status: statusFilter || undefined, supplierId: supplierFilter || undefined });
  ...
  className="flex items-center gap-2 px-4 py-2 bg-[#33cbcc] text-white  text-sm hover:bg-[#2bb5b6]">
  <Add01Icon size={16} />
  ...
  className={`flex items-center gap-1.5 px-4 py-2  text-sm font-medium transition-colors ${tab === t.key ? 'bg-[#283852]/10 text-[#283852]' : 'text-gray-600 hover:bg-gray-100'}`}>
  ...
  <div key={s.label} className="bg-white  border p-4">
  ...
  className="border  px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#33cbcc]/30 bg-white">
  ...
  className="p-1.5 hover:bg-gray-100  text-gray-500" title="Voir">
  <ViewIcon size={15} />
  </button>
  {inv.status === 'DRAFT' && (
  <button onClick={() => setEditInvoice(inv)}
  className="p-1.5 hover:bg-gray-100  text-gray-500 ml-1" title="Modifier">
  <PencilIcon size={15} />
  </button>
  )}
  </td>
  ...
  <div key={s.id} className="bg-white  border p-5 hover:shadow-md transition-shadow">
  <div className="flex items-start justify-between mb-3">
  <div className="flex items-center gap-3">
  <div className="w-10 h-10  bg-[#283852]/10 flex items-center justify-center">
  <DeliveryTruck01Icon size={18} className="text-[#283852]" />
  </div>
  <div>
  ...
  className="p-1.5 hover:bg-gray-100  text-gray-400"><PencilIcon size={14} /></button>
  <button onClick={() => { if (confirm('Supprimer ce fournisseur ?')) deleteSupplier.mutate(s.id); }}
  className="p-1.5 hover:bg-[#283852]/10  text-gray-400 hover:text-[#283852]"><Delete02Icon size={14} /></button>
  </div>
  </div>
