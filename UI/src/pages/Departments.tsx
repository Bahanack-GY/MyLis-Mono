...
const CreateDepartmentModal = ({ onClose }: { onClose: () => void }) => {
  ...
  <h3 className="text-base font-bold text-[#1c2b3a]">{t('departments.create.title')}</h3>
  </div>
  ...
  : 'border-[#e5e8ef] hover:border-[#b0bac9] bg-white'
  }`}
  >
  ...
  className={`w-10 h-10  flex items-center justify-center border-2 transition-all ${
  isActive ? 'border-[#1c2b3a] scale-110' : 'border-transparent '
  }`}
  ...
  <span className="text-[10px] font-semibold text-[#8892a4]">+{selectedMembers.length - 3}</span>
  )}
  </div>
  )}
  </div>
