...
interface ModalProps {
  ...
  /** Extra Tailwind classes for the panel, controls width e.g. "w-full max-w-md" */
  className?: string;
...
/**
 * Accessible right-side drawer shell.
 *
 * Provides:
...
const Modal = ({
  ...
  // Focus trap: Tab / Shift+Tab cycle through focusable elements in the panel
  ...
  animate={{ opacity: 1 }}
  exit={{ opacity: 0 }}
  className={`fixed inset-0 ${zIndex} flex justify-end bg-black/30`}
  onClick={onClose}
  // Backdrop is presentation-only; the dialog role is on the panel div
  aria-hidden="true"
  >
  <motion.div
  ref={contentRef}
  ...
  aria-hidden="false"
  className={`bg-white h-full flex flex-col border-l border-[#e5e8ef] overflow-hidden ${className}`}
  >
  {children}
  </motion.div>
  </motion.div>
