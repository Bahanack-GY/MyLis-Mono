...
export default function PlatformChatButton() {
  ...
  className="fixed bottom-24 right-6 z-[110] w-[360px] flex flex-col  shadow-2xl overflow-hidden"
  ...
  <div className="bg-white shadow-sm   px-3 py-3 flex items-center gap-1">
  {[0, 1, 2].map(i => (
  <motion.div
  key={i}
  ...
  disabled={loading}
  className="flex-1 text-sm  border border-gray-200 px-3 py-2 outline-none focus:border-[#33cbcc] transition-colors disabled:bg-gray-50"
  />
  <button
  onClick={send}
  disabled={!input.trim() || loading}
  className="w-9 h-9  flex items-center justify-center shrink-0 transition-all disabled:opacity-40"
