...
function ClientHealthItem({ clientId, clientName, onSelect }: { clientId: string; clientName: string; onSelect: () => void }) {
  ...
  whileTap={{ scale: 0.98 }}
  className={`w-full p-3  transition-all text-left ${config.bgColor}`}
  >
  ...
  <div className={`inline-flex items-center gap-1.5 mt-1 px-2 py-0.5  ${config.textColor} bg-white/80`}>
  <Icon size={12} />
